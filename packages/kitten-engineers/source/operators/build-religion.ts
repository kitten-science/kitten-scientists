...
export const BuildReligionFactory = function* (
  items: Iterable<ReligionUpgrade | TranscendenceUpgrade | ZiggurathUpgrade>,
) {
  for (const item of items) {
    yield class extends TreeNode<Operator> implements Operator {
      ...
      requires = coalesceArray([
        ...
        ...(item === "solarRevolution" ? ["epiphany" as const, "transcendenceTier" as const] : []),
      ]);
