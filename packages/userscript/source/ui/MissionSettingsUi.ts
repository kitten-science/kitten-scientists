...
export class MissionSettingsUi extends SettingsPanel<MissionSettings> {
  ...
  constructor(host: UserScript, settings: MissionSettings) {
    ...
    this._list.addEventListener("enableAll", () => {
      this._missions.forEach(item => (item.settings.enabled = true));
      this.refreshUi();
    });
    this._list.addEventListener("disableAll", () => {
      this._missions.forEach(item => (item.settings.enabled = false));
      this.refreshUi();
    });
    this._list.addEventListener("reset", () => {
      this.settings.load(new MissionSettings());
      this.refreshUi();
    });

    const items = [];
    for (const [name, setting] of objectEntries(this.settings.items)) {
      const label = this._host.engine.i18n(`$space.${name}.label`);
