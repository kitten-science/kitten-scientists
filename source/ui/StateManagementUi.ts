...
export class StateManagementUi extends SettingsPanel<StateSettings> {
		...
		this.addChildContent(
			new SettingsList(this, {
				...
				new SettingListItem(
					...
					this.host.engine.i18n("state.noConfirm"),
				),
				...
				new SettingListItem(
					...
					this.host.engine.i18n("state.compress"),
				),
			]),
		);
	...
	private _refreshGameList(parent: UiComponent) {
		...
		this.gameList.addChildren(
			this.games
				.sort(
					(a, b) =>
						new Date(a.unwrap().timestamp).getTime() -
						new Date(b.unwrap().timestamp).getTime(),
				)
	...
	async import(parent: UiComponent) {
		...
		const makeImportLabel = () =>
			this.host.engine.i18n("state.importedState", [
	...
	loadAutoSave() {
		if (this.host.engine.isLoaded) {
			console.info(
				...cl(
					"Not attempting to load Auto-Save, because a state is already loaded.",
				),
			);
			return;
		}
		...
		const requiresUiRebuild =
			this.host.engine.settings.ksColumn.enabled !==
			state.engine.ksColumn.enabled;
