  ...
  constructor(parent: UiComponent, settings: LogFilterSettings) {
    ...
    this.addChildContent(
      new SettingsList(this, {
        ...
        new HeaderListItem(this, "Kittens Game"),
        new SettingsList(this).addChildren(
          FilterItemsGame.map(item => {
            return { label: this.host.engine.i18n(`$console.filter.${item}`), name: item };
          })
            .sort((a, b) => a.label.localeCompare(b.label))
            .map(
              item =>
                new SettingListItem(this, this.setting.filtersGame[item.name], item.label, {
                  onCheck: () => {
                    this.host.engine.imessage("filter.enable", [item.label]);
                  },
                  onUnCheck: () => {
