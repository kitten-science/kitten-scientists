...
export class UserInterface extends UiComponent {
	...
	constructor(host: KittenScientists) {
		...
		this._sections = [
			new BonfireSettingsUi(
				this,
				engine.bonfireManager.settings,
				engine.settings.locale,
			),
			new VillageSettingsUi(
				this,
				engine.villageManager.settings,
				engine.settings.locale,
			),
			new ScienceSettingsUi(
				this,
				engine.scienceManager.settings,
				engine.settings.locale,
			),
			new WorkshopSettingsUi(
				this,
				engine.workshopManager.settings,
				engine.settings.locale,
			),
			new ResourcesSettingsUi(
				this,
				engine.settings.resources,
				engine.settings.locale,
			),
			new TradeSettingsUi(
				this,
				engine.tradeManager.settings,
				engine.settings.locale,
			),
			new ReligionSettingsUi(
				this,
				engine.religionManager.settings,
				engine.settings.locale,
			),
			new SpaceSettingsUi(
				this,
				engine.spaceManager.settings,
				engine.settings.locale,
			),
			new TimeSettingsUi(
				this,
				engine.timeManager.settings,
				engine.settings.locale,
			),
		...
		const usingKSColumn =
			engine.settings.ksColumn.enabled && game.colorScheme !== "sleek";
		if (usingKSColumn) {
			$("#rightColumn").after(
		...
		if (right.length === 0) {
			...
			const optionsPageContent = $(
				"#optionsPage .full-screen-position .page .page-content",
			);
			if (optionsPageContent.length === 0) {
				console.warn(
					...cl(
						"Unable to find right column to inject UI into. Running headless.",
					),
				);
			} else {
	...
	requestRefresh(_withChildren = true, depth = 0, _trace = false) {
		if (this._needsRefresh) {
			if (this._refreshTimeout === undefined) {
				console.error(
					...cl(
						"User interface claims to have a refresh pending, but there isn't.",
					),
				);
			}
			return;
