...
export type WorkshopResourceSettings = Record<
	ResourceCraftable,
	CraftSettingsItem
>;
...
export class WorkshopSettings extends SettingTrigger {
	...
	static validateGame(game: GamePage, settings: WorkshopSettings) {
		...
		for (const _ of missingInSettings) {
			console.warn(
				...cl(`The workshop craft '${_}' is not tracked in Kitten Scientists!`),
			);
		}
		for (const _ of redundantInSettings) {
			console.warn(
				...cl(`The workshop craft '${_}' is not an upgrade in Kittens Game!`),
			);
		}
	...
	load(settings: Maybe<Partial<WorkshopSettings>>) {
		...
		this.shipOverride.enabled =
			settings.shipOverride?.enabled ?? this.shipOverride.enabled;
	}
}
