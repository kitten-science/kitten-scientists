...
export class TradeSettings extends SettingTrigger {
	...
	load(settings: Maybe<Partial<TradeSettings>>) {
		...
		consumeEntriesPedantic(this.races, settings.races, (race, item) => {
			...
			race.seasons.autumn.enabled =
				item?.seasons.autumn.enabled ?? race.seasons.autumn.enabled;
			race.seasons.spring.enabled =
				item?.seasons.spring.enabled ?? race.seasons.spring.enabled;
			race.seasons.summer.enabled =
				item?.seasons.summer.enabled ?? race.seasons.summer.enabled;
			race.seasons.winter.enabled =
				item?.seasons.winter.enabled ?? race.seasons.winter.enabled;
		});
