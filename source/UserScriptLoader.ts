...
export class UserScriptLoader {
	...
	static tryEngineStateFromSaveData(
		saveDataKey: string,
		saveData: unknown,
	): unknown {
		...
		if (!(saveDataKey in saveDataProxy)) {
			console.debug(
				...cl(`Failed: \`${saveDataKey}\` not found in save data.`),
			);
			return undefined;
		}
		...
		if (!("state" in ksData)) {
			console.debug(
				...cl(`Failed: \`${saveDataKey}.state\` not found in save data.`),
			);
			return undefined;
		}
	...
	async waitForGame<TUserScript>(
		...
		if (UserScriptLoader._isGameLoaded()) {
			...
			const gameLanguage = localStorage[
		...
		if (
			isNil(this._gameStartSignal) &&
			typeof UserScriptLoader.window.dojo !== "undefined"
		) {
			this._gameStartSignal = new Promise((resolve) => {
				this._gameStartSignalResolver = resolve;
			});

			const subGameStart = UserScriptLoader.window.dojo.subscribe(
				"game/start",
				() => {
					console.debug(
						...cl(
							`'game/start' signal caught. Fast-tracking script load for '${saveDataKey}'...`,
						),
					);
		...
		console.debug(
			...cl(`Waiting for game... (timeout: ${Math.round(timeout / 1000)}s)`),
		);
	...
	private static _isGameLoaded(): boolean {
		return (
			...
			(UserScriptLoader.window.document
				.getElementById("game")
				?.checkVisibility() ??
				false) &&
