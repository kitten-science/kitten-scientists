...
> & {
	meta: [
		UnsafeMeta<UnsafeChronoForgeUpgrade>,
		UnsafeMeta<UnsafeVoidSpaceUpgrade>,
	];
	game: GamePage;
	/**
...
export type AccelerateTimeBtnController = ButtonModernController & {
	fetchModel: (options: unknown) => ReturnType<
		ButtonModernController["fetchModel"]
	> & {
		toggle: {
			title: string;
			tooltip: string;
...
export type AccelerateTimeBtn =
	ButtonModern<UnsafeAccelerateTimeButtonOptions> & {
		renderLinks: () => void;
		toggle?: Link;
		update: () => void;
	};
...
export type ShatterTCBtnController =
	ButtonModernController<UnsafeShatterTCBtnModel> & {
		defaults: () => ReturnType<ButtonModernController["defaults"]> & {
...
export type FixCryochamberBtnController =
	ButtonModernController<UnsafeFixCryochamberBtnModel> & {
		defaults: () => UnsafeFixCryochamberBtnModel;
		buyItem: (
			model: UnsafeFixCryochamberBtnModel,
			event: Event,
		) => UnsafeBuyItemResult;
...
export type TimeTab = Tab<
	| Panel<TimeControlWgt>
	| Panel<ResetWgt>
	| Panel<ChronoforgeWgt>
	| Panel<VoidSpaceWgt>
> & {
	container: null;
...
export type UnsafeVoidSpaceUpgrade = {
	...
	unlockScheme?: {
		name: "arctic" | "dune" | "fluid" | "space" | "vessel";
		threshold: number;
	};
