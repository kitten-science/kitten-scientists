...
export type BiomeBtn = ButtonModern<UnsafeBiomeButtonOptions> & {
	...
	getTooltipHTML: () => (
		...
		model: UnsafeBiomeBtnModel,
	) => string;
};

export type UpgradeHQController =
	BuildingStackableBtnController<UnsafeUpgradeHQBtnModel> & {
		...
		incrementValue: (model: {
			metadata: Metadata<UnsafeUpgradeHQBtnModel>;
		}) => void;
...
export type UpgradeExplorersController = BuildingStackableBtnController & {
	...
	incrementValue: (model: {
		metadata: Metadata<UnsafeUpgradeExplorersBtnModel>;
	}) => void;
	hasSellLink: (model: unknown) => false;
...
export type VillageButtonController<
...
export type FestivalButtonController =
	VillageButtonController<UnsafeFestivalButtonModel> & {
