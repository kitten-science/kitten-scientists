...
export type Metadata<
	TMeta extends Record<string, unknown> | unknown = unknown,
> = {
	meta: TMeta;
...
> = Metadata<TMeta> & {
	...
	set<TAttr extends keyof TMetaCache>(
		attr: TAttr,
		val: TMetaCache[TAttr],
	): void;
};
...
export type RefineCatnipButtonController = ButtonModernController & {
	...
	fetchModel: <
		TModel extends
			UnsafeRefineCatnipButtonModel = UnsafeRefineCatnipButtonModel,
	>(
		options: unknown,
	) => TModel;
	handleX100Click: <
...
export type RefineCatnipButton =
	ButtonModern<UnsafeRefineCatnipButtonOptions> & {
...
export type BuildingBtnModernController<
	TModel extends
		| UnsafeBuildingBtnModernModel<AllBuildingStackableBtnOptions>
		| unknown = unknown,
> = BuildingStackableBtnController<TModel> & {
	...
	incrementValue: (model: {
		metadata: Metadata<TModel>;
		metaAccessor: unknown;
	}) => void;
};
