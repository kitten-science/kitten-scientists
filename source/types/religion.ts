...
> & {
	...
	getCorruptionPerTickProduction: (
		pretendExistNecrocorn?: boolean | undefined,
	) => number;
...
export type TransformBtnController<
	...
	TControllerOpts extends Record<string, unknown> | undefined = Record<
		string,
		unknown
	>,
...
export type MultiLinkBtn<TOpts extends AllMultiLinkBtnOptions> =
	ButtonModern<TOpts> & {
		all?: Link;
		half?: Link;
		fifth?: Link;
...
export type RefineTearsBtnController = ButtonModernController & {
	...
	buyItem: (
		model: UnsafeRefineTearsBtnModel,
		event: Event,
		count: number,
	) => UnsafeBuyItemResult;
	refine: () => void;
...
export type PactsWGT = IChildrenAware<
	BuildingStackableBtn<UnsafePactsButtonOptions>
> &
	IGameAware & {
...
export type PactsBtnController<
	TModel extends UnsafePactsBtnModel | unknown = unknown,
> = BuildingStackableBtnController<TModel> & {
	...
	buyItem: (
		...
		| UnsafeBuyItemResult
		| ReturnType<BuildingStackableBtnController<TModel>["buyItem"]>;
...
> = UnsafeZigguratBtnModelDefaults &
...
> = UnsafeReligionBtnModelDefaults &
	UnsafeBuildingStackableBtnModel<TModelOptions>;
...
> = UnsafeTranscendenceBtnModelDefaults &
	UnsafeBuildingStackableBtnModel<TModelOptions>;
...
export type UnsafeTransformBtnModel<
	TModelOptions extends Record<string, unknown> | undefined = Record<
		string,
		unknown
	>,
> = {
