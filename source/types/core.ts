...
export type TabManager<TMeta extends UnsafeMeta<unknown> | unknown = unknown> =
	Control & {
		...
		getMeta: <TMeta extends UnsafeMeta>(
			name: string,
			metadata: Array<TMeta>,
		) => TMeta | undefined;
		loadMetadata: (
...
> = {
	...
	hasResources: (
		model: { prices?: Maybe<Array<Price>> },
		prices?: Array<Price>,
	) => boolean;
	/**
	...
	getPrices: <T extends { prices?: Maybe<Array<Price>> }>(
		model: T,
	) => Array<Price>;
 ...
 */
export type Button<TOpts extends AllButtonOptions | unknown = unknown> =
	Control & {
		model: TOpts extends { controller: { fetchModel: AnyFunctionReturning } }
			? ReturnType<TOpts["controller"]["fetchModel"]>
			: unknown;
		controller: TOpts extends { controller: unknown }
			? TOpts["controller"]
			: unknown;
		game: GamePage;
		...
		addLinkList: (
			links: Array<UnsafeLinkModel>,
		) => Record<string, UnsafeLinkResult>;
	};
...
export type ButtonModern<
	TOpts extends AllButtonModernOptions | unknown = unknown,
> = Button<TOpts> & {
	afterRender: () => void;
...
export type BuildingBtnController<
	TModel extends
		| UnsafeBuildingBtnModernModel<AllBuildingBtnOptions>
		| unknown = unknown,
> = ButtonModernController<TModel> & {
	...
	getEffects: (
		model: TModel,
	) => TModel extends { effects: unknown } ? TModel["effects"] : unknown;
	getTotalEffects: (
		model: TModel,
	) => TModel extends { totalEffectsCached: unknown }
		? TModel["totalEffectsCached"]
		: unknown;
...
export type BuildingStackableBtn<
	TOpts extends AllBuildingStackableBtnOptions | unknown = unknown,
> = BuildingBtn<TOpts> & {
...
export type BuildingResearchBtn<TOpts extends AllBuildingResearchBtnOptions> =
	BuildingBtn<TOpts>;
...
export type UnsafeBuildingBtnModel<
	TModelOptions extends UnsafeButtonOptions | unknown = unknown,
