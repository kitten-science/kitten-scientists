...
export type PrestigeBtnController = BuildingNotStackableBtnController & {
	getMetadata: <TButtonModel extends UnsafeBuildingBtnModel>(
		model: TButtonModel,
	) => UnsafePerk;
	buyItem: (model: unknown, event: unknown) => UnsafeBuyItemResult;
	updateVisible: <TButtonModel extends UnsafeBuildingBtnModel>(
		model: TButtonModel,
	) => void;
};

export type BurnParagonBtnController = ButtonModernController & {
	updateVisible: <TButtonModel extends UnsafeButtonModel>(
		model: TButtonModel,
	) => void;
};
