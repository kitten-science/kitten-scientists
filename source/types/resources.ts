...
export type ResourceManager = TabManager & {
	...
	resourceMap: Record<
		Resource,
		UnsafeResource & { value: number; unlocked: boolean }
	>;
	...
	addResPerTick: (
		name: Resource,
		value: number,
	) => ReturnType<ResourceManager["addRes"]>;
	addResEvent: (
		name: Resource,
		value: number,
	) => ReturnType<ResourceManager["addRes"]>;
	getAmtDependsOnStock: (
		from: Array<{ res: Resource; amt: number }>,
		amt: number,
	) => number;
	update: () => void;
	...
	addBarnWarehouseRatio: <
		TEffects extends Partial<Record<BuildingEffect, number>>,
	>(
		effects: TEffects,
	) => TEffects;
